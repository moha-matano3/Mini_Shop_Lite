...
@section('content')
    ...
    <div class="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-8">
        <h1 class="text-2xl font-bold text-gray-800 mb-6 text-center">✏️ Edit Product</h1>
        ...
        <form method="POST" action="{{ url('update_product', $data->id) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <img src="{{ asset('product/'.$data->product_img) }}" alt="{{ $data->name }}" 
                     class="w-24 h-24 object-cover rounded-md shadow-md mb-2">
            </div>

            <!-- Update Product Image -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Update Product Image</label>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" value="{{ old('price', $data->price) }}"
                       class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 px-4 py-2"
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 px-4 py-2"
                          placeholder="Enter product description" required>{{ old('description', $data->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="category" 
                    ...
                    @foreach ($categories as $cat)
                        <option value="{{ $cat->id }}" 
                                {{ old('category', $data->category_id) == $cat->id ? 'selected' : '' }}>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
